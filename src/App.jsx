...
export default function App() {
  ...
  return (
    <div style={{ background: c.bg, minHeight: '100vh', fontFamily: sans, color: c.text }}>
      {/* HEADER */}
      <div style={{ padding: '1.4rem 2rem', borderBottom: `1px solid ${c.borderLight}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.8rem' }}>
          <h1 style={{ fontFamily: serif, fontSize: '1.6rem', fontWeight: 700, letterSpacing: '-0.025em', color: c.green, margin: 0 }}>The Wisdom Hub</h1>
          <div style={{ fontFamily: sans, fontSize: '0.75rem', color: c.textMuted, letterSpacing: '0.02em' }}>
            {ALL_HIGHLIGHTS.length.toLocaleString()} highlights &middot; {UNIQUE_BOOKS} books &middot; {ALL_MODELS.length} models &middot; 12.6M words
          </div>
        </div>
      </div>

      {/* SOLVER */}
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3.5rem 2rem 3rem' }}>
        <h2 style={{ fontFamily: serif, fontSize: '2.4rem', fontWeight: 700, letterSpacing: '-0.02em', color: c.text, margin: '0 0 0.5rem 0' }}>What are you working through?</h2>
        <p style={{ fontFamily: serif, fontSize: '1rem', fontStyle: 'italic', color: c.textMuted, margin: '0 0 1.8rem 0' }}>
          Your library of {ALL_HIGHLIGHTS.length.toLocaleString()} highlights will surface the wisdom you need.
        </p>
        <div style={{ display: 'flex', gap: '0.7rem', flexWrap: 'wrap' }}>
          <input
            style={{ flex: 1, minWidth: 250, padding: '1rem 1.2rem', background: c.bgCard, border: `1.5px solid ${c.border}`, borderRadius: 10, color: c.text, fontFamily: serif, fontSize: '1rem', outline: 'none' }}
          ...
          <button
            ...
            style={{ padding: '1rem 2rem', background: solverLoading ? c.textMuted : c.green, color: '#fff', border: 'none', borderRadius: 10, fontFamily: sans, fontSize: '0.88rem', fontWeight: 600, cursor: solverLoading ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
          >
        ...
        {solverLoading && (
          <div style={{ fontFamily: serif, fontStyle: 'italic', color: c.textMuted, padding: '2.5rem 0', textAlign: 'center', fontSize: '1rem' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 2rem 3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem', gap: '1rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: c.textMuted }}>Browse by mental model</span>
          <input
            style={{ width: 200, padding: '0.5rem 0.8rem', background: c.bgCard, border: `1px solid ${c.border}`, borderRadius: 6, color: c.text, fontSize: '0.82rem', outline: 'none', fontFamily: sans }}
            placeholder="Filter..."
            value={search}
        ...
        {/* MODEL LIST */}
        <div style={{ background: c.bgCard, border: `1px solid ${c.borderLight}`, borderRadius: 12, overflow: 'hidden' }}>
          {filteredModels.map((m, i) => (
            <div
              ...
              style={{
                ...
                padding: '1rem 1.4rem',
                borderBottom: i < filteredModels.length - 1 ? `1px solid ${c.borderLight}` : 'none',
                cursor: 'pointer',
                background: activeModel === m.name ? c.greenFaint : 'transparent',
                borderLeft: activeModel === m.name ? `3px solid ${c.green}` : '3px solid transparent',
                transition: 'background 0.15s',
              }}
              onMouseEnter={(e) => { if (activeModel !== m.name) e.currentTarget.style.background = c.bgWarm }}
              onMouseLeave={(e) => { if (activeModel !== m.name) e.currentTarget.style.background = 'transparent' }}
            >
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.7rem', minWidth: 0, flex: 1 }}>
                <span style={{ fontFamily: serif, fontSize: '1.05rem', fontWeight: 700, color: c.text, whiteSpace: 'nowrap' }}>{m.name}</span>
                <span style={{ fontFamily: serif, fontSize: '0.85rem', fontStyle: 'italic', color: c.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.description}</span>
              </div>
              <span style={{ fontFamily: mono, fontSize: '0.78rem', color: c.textMuted, whiteSpace: 'nowrap' }}>{m.displayCount || m.count}</span>
            </div>
          ))}
        </div>
